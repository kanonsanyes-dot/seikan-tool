...
def add_customer():
    ...
    if name and not Customer.query.filter_by(customer_name=name).first():
        db.session.add(Customer(customer_name=name, is_active=True)); db.session.commit(); cache_service.clear(); flash("出荷先を追加しました。", "success")
...
def edit_customer(id):
    ...
    if c:
        c.customer_name=request.form.get("customer_name",c.customer_name); c.is_active=bool(request.form.get("is_active")); db.session.commit(); cache_service.clear(); recheck_all(Order.query.all()); db.session.commit()
...
def delete_customer(id):
    c=db.session.get(Customer,id)
    if c: db.session.delete(c); db.session.commit(); cache_service.clear(); flash("出荷先を削除しました。", "success")
    return redirect(url_for("masters.customers"))
...
def add_product():
    ...
    if name and not Product.query.filter_by(product_name=name).first():
        db.session.add(Product(product_name=name, process_product_name=request.form.get("process_product_name"), is_active=True)); db.session.commit(); cache_service.clear(); flash("品名を追加しました。", "success")
...
def edit_product(id):
    ...
    if p:
        p.product_name=request.form.get("product_name",p.product_name); p.process_product_name=request.form.get("process_product_name"); p.is_active=bool(request.form.get("is_active")); db.session.commit(); cache_service.clear(); recheck_all(Order.query.all()); db.session.commit(); flash("品名を更新しました。", "success")
    return redirect(url_for("masters.products"))

@masters_bp.route("/products/<int:id>/delete", methods=["POST"])
def delete_product(id):
    p=db.session.get(Product,id)
    if p: db.session.delete(p); db.session.commit(); cache_service.clear(); flash("品名を削除しました。", "success")
    return redirect(url_for("masters.products"))
...
def add_standard():
    s=ProductProcessStandard(product_name=request.form.get("product_name"), process_product_name=request.form.get("process_product_name"), process_name=request.form.get("process_name"), process_order=int(request.form.get("process_order",1)), standard_time_min=float(request.form.get("standard_time_min",0) or 0), daily_capacity=int(request.form.get("daily_capacity",0) or 0), lot_size=int(request.form.get("lot_size",0) or 0), is_active=True, remarks=request.form.get("remarks"))
    db.session.add(s); db.session.commit(); cache_service.clear(); flash("工程標準を追加しました。", "success")
...
def delete_standard(id):
    ...
    if s: db.session.delete(s); db.session.commit(); cache_service.clear(); flash("工程標準を削除しました。", "success")
    return redirect(url_for("masters.standards"))
...
def add_capacity():
    c=ProcessCapacity(process_name=request.form.get("process_name"), work_date=datetime.fromisoformat(request.form.get("work_date")).date(), available_hours=float(request.form.get("available_hours",8) or 8), workers=int(request.form.get("workers",1) or 1), overtime_hours=float(request.form.get("overtime_hours",0) or 0), capacity_quantity=int(request.form.get("capacity_quantity",0) or 0), remarks=request.form.get("remarks"))
    db.session.add(c); db.session.commit(); cache_service.clear(); flash("工程キャパを追加しました。", "success")
...
def delete_capacity(id):
    ...
    if c: db.session.delete(c); db.session.commit(); cache_service.clear(); flash("工程キャパを削除しました。", "success")
    return redirect(url_for("masters.capacity"))
